...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help)
      ...
      echo ""
      echo "Examples:"
      echo "  $0 --local                                    # Create table in local DynamoDB"
      echo "  $0 --table-name microrealestate-dev           # Create dev table in AWS"
      echo "  $0 --table-name microrealestate-prod --region us-west-2  # Create prod table"
      exit 0
      ;;
    *)
